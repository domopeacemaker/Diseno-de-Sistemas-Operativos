...
static ssize_t speaker_read(struct file *file, char __user *buf,
                         size_t count, loff_t *ppos)
{
    /*char ch;
...
static int r_GPIO_config(void)
{
    ...
    if ((res=gpio_request_one(GPIO_SPEAKER, GPIOF_INIT_LOW, "Speaker :D"))) //He quitado un parentesis Errr
            return res;
